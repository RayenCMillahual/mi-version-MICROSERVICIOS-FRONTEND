...
export class ProductsService {
  ...
  // ✅ AGREGAR: Verificar que hay suficiente stock ANTES de reducir
  if (reservation.product.stock < reservation.quantity) {
    throw new BadRequestException(
      `Insufficient stock. Available: ${reservation.product.stock}, Required: ${reservation.quantity}`
    );
  }

  // ✅ USAR TRANSACCIÓN para atomicidad
  return await this.productRepo.manager.transaction(async (transactionManager) => {
    ...
    await transactionManager.save(ProductReservation, reservation);

    // Verificar que el stock no quedó negativo (por las dudas)
    const updatedProduct = await transactionManager.findOne(Product, {
      where: { id: reservation.productId },
    });

    if (updatedProduct && updatedProduct.stock < 0) {
      throw new BadRequestException('Stock operation resulted in negative stock');
    }
  ...
  // ✅ AGREGAR: Método para revertir reservas (compensating transaction)
async revertReservation(reservationId: string) {
  const reservation = await this.reservationRepo.findOne({
    where: { id: reservationId },
    relations: ['product'],
  });
  ...
  if (!reservation.isCompleted) {
    // Si no estaba completada, solo la eliminamos
    await this.reservationRepo.remove(reservation);
    return { 
  ...
  return await this.productRepo.manager.transaction(async (manager) => {
    ...
    // Eliminar la reserva
    await manager.remove(ProductReservation, reservation);
    ...
    return { 
      message: 'Reservation reverted and stock restored',
